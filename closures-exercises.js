...
function guessingGame(amount) {
    var answer = Math.floor(Math.random()*11) // random number
    var guesses = 0  //number of guesses to be incremented
    var completed = false  // game finished?
    console.log(answer)
    return function(guess){ //inner function
      if(completed = false){   // is the game completed?
          guesses++
      if (guess === answer ){
         completed = true;
        return "ding ding"
        ...
        completed = true
        return "ran out of chances"
      }
       else if(guess > answer){
        console.log("you have had this many guesses " + guesses)
        return "too high"
      } else if (guess < answer){
      console.log("you have had this many guesses " + guesses)
        return "too low!"
